...
try {
    ...
    # Lock the inventory of non-ASCII sources. These files carry the accented
    # fixtures the coverage tests assert on, so "normalizing" any of them to
    # BOM-less UTF-8 silently changes what MSVC reads. Adding a new non-ASCII
    # source is allowed, but it has to be a deliberate edit to this list.
    $expectedNonAscii = [ordered]@{
        "CppCoverageTest/CodeCoverageRunnerTest.cpp"    = "ansi"
